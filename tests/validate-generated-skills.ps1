...
try {
    ...
    $claudeZip = Join-Path $repoRoot 'release/turning-ideas-into-projects-claude-code-0.4.0.zip'
    ...
    $uniClawMainZip = Join-Path $repoRoot 'release/turning-ideas-into-projects-uniclaw-0.4.0.zip'
    ...
    $uniClawChildZip = Join-Path $repoRoot 'release/orchestrating-multi-model-work-uniclaw-0.4.0.zip'
    ...
    $claudeDesktopMainZip = Join-Path $repoRoot 'release/turning-ideas-into-projects-claude-desktop-0.4.0.zip'
    $claudeDesktopMainDestination = Join-Path $tempRoot 'claude-desktop/turning-ideas-into-projects'
    Expand-Archive -LiteralPath $claudeDesktopMainZip -DestinationPath $claudeDesktopMainDestination -Force
    $claudeDesktopMainSkill = Join-Path $claudeDesktopMainDestination 'turning-ideas-into-projects'
    $targets.Add($claudeDesktopMainSkill)
    Get-ChildItem -LiteralPath (Join-Path $claudeDesktopMainSkill 'references/bundled-skills') -Directory | ForEach-Object { $targets.Add($_.FullName) }

    $claudeDesktopChildZip = Join-Path $repoRoot 'release/orchestrating-multi-model-work-claude-desktop-0.4.0.zip'
    $claudeDesktopChildDestination = Join-Path $tempRoot 'claude-desktop/orchestrating-multi-model-work'
    Expand-Archive -LiteralPath $claudeDesktopChildZip -DestinationPath $claudeDesktopChildDestination -Force
    $targets.Add((Join-Path $claudeDesktopChildDestination 'orchestrating-multi-model-work'))
