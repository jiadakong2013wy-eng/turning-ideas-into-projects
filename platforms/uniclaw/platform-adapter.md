...
| Model route | Use the shared capability-based selector, but recommend only models observed in the current client. Do not invent a fixed brand mapping. A configured or selected model is `model_requested`; it becomes `model_actual` only when the running task exposes that value. |
