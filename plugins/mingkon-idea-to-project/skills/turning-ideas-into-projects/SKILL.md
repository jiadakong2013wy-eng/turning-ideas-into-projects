...
2. **REQUIRED SUB-SKILL:** in Plugin mode use the bundled `leader` resolved in the current host's plugin namespace by the dependency gate; in standalone authoring mode use the separately installed `leader`. Freeze only the approved next phase as one Agent contract.
...
- Resolve siblings by the current host and installed manifest, not the source directory name: Codex Plugin mode requires `mingkon-idea-to-project:leader` and `mingkon-idea-to-project:orchestrating-multi-model-work`; Claude Code Plugin mode requires `turning-ideas-into-projects:leader` and `turning-ideas-into-projects:orchestrating-multi-model-work`. This applies to direct GitHub marketplace installs as well as ZIP installs. In either Plugin mode it must not use a generic `leader` installation to satisfy the bundled dependency gate. Flat or single-Skill packages follow their platform adapter's bundled-resource mapping.
...
For a phase that needs external research, more than one model/role, long-running child work, or implementation-independent review, conditionally load the bundled `orchestrating-multi-model-work` in the namespace resolved above after this leader contract is approved. It coordinates only versioned child receipts for the frozen phase. This Skill remains the unique lifecycle owner: it alone decides discovery, GO/PIVOT/HOLD/STOP, placement, contract changes, native goal creation, central governance writes, acceptance routing, and user-facing escalation.

Before composing the adoption notice, read the resolved child's `references/model-routing.md` without dispatching work. Then notify the user of the reason, active contract version, observed platform/subscription, proposed role-to-model route, capability gaps, and cost effect. Offer `推荐适配`, `质量优先`, `省额度/速度优先`, and `自定义每个角色`; also accept direct per-role overrides. At first use, wait for a choice through the available UI or end the reply after the Markdown menu. An explicit route or previously approved project preference satisfies this choice point; later unchanged phases need only a notice. Unknown cost stops the affected dispatch; extra paid credits require explicit authorization. A user selection overrides a recommendation only when the capability is observed and the frozen contract still preserves its evidence, cost, and independence gates. Otherwise explain the conflict and return to `leader` to re-freeze the affected contract boundary. Never silently substitute an unavailable model or remove a model-specific independent review.
